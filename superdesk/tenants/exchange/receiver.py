...
async def _deliver(item: dict | str, source_tenant: str, auto_fetch=False, desk=None, stage=None):
    doc: dict = json.loads(item) if isinstance(item, str) else item
    ...
    parsed = parser._transform_from_ninjs(doc)
    parsed.setdefault("guid", doc.get("guid"))
    ...
    if doc.get("renditions"):
        parsed["renditions"] = doc["renditions"]
    for key, association in (doc.get("associations") or {}).items():
    ...
    logger.info("exchange delivered item=%s source=%s target=%s", doc.get("guid"), source_tenant, target.id)
    ...
    if auto_fetch:
        ...
        try:
            ...
            logger.exception("exchange auto-fetch failed item=%s target=%s", doc.get("guid"), target.id)
